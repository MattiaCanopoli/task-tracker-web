...
public class TaskService {
	...
	/**
	 * Retrieves all {@link Task} entities created by the specified user and
	 * matching the given status name.
	 * <p>
	 * If the provided status name does not correspond to an existing status, an
	 * {@link IllegalArgumentException} is thrown.
	 * </p>
	 *
	 * @param userId
	 *            the ID of the user who created the tasks
	 * @param statusName
	 *            the name of the status used to filter tasks
	 * @return a list of {@link Task} entities created by the specified user
	 *         with the given status; the list may be empty if no tasks match
	 * @throws IllegalArgumentException
	 *             if the provided status name is not valid
	 */
	...
	/**
	 * Updates the status of the provided {@link Task} based on the data from
	 * the given {@link DTOTask}.
	 * <p>
	 * If the provided status ID does not correspond to an existing
	 * {@link Status}, an {@link IllegalArgumentException} is thrown.
	 * </p>
	 * <p>
	 * If the new status ID is {@code 3} (representing "DONE"), the
	 * {@code completedAt} field is also set to the current {@link Timestamp}.
	 * </p>
	 * <p>
	 * The updated {@link Task} is then persisted to the task_tracker_web
	 * database.
	 * </p>
	 *
	 * @param task
	 *            the {@link Task} entity to be updated
	 * @param dto
	 *            the {@link DTOTask} containing the new status ID
	 * @return the updated {@link Task} entity
	 * @throws IllegalArgumentException
	 *             if the provided status ID is invalid
	 */
	...
	/**
	 * Updates the description of the provided {@link Task} based on the data
	 * from the given {@link DTOTask}.
	 * <p>
	 * If the provided {@code description} is null or empty, an
	 * {@link IllegalArgumentException} is thrown.
	 * </p>
	 * <p>
	 * After validation, the description of the provided {@link Task} is
	 * updated, and the task is then persisted to the task_tracker_web database.
	 * </p>
	 *
	 * @param task
	 *            the {@link Task} entity to be updated
	 * @param dto
	 *            the {@link DTOTask} containing the new {@code description}
	 * @return the updated {@link Task} entity
	 * @throws IllegalArgumentException
	 *             if the provided {@code description} is null or empty
	 */
	...
	/**
	 * Marks the given {@link Task} as {@code DELETED} by updating its
	 * {@link Status} to {@code 4} ("DELETED"), setting the {@code deletedAt}
	 * field to the current {@link Timestamp}, and the {@code isDeleted} field
	 * to {@code true}.
	 * <p>
